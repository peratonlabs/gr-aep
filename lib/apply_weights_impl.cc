...
apply_weights_impl::apply_weights_impl(int M, int num_sc, int num_avg)
    : gr::block(
          ...
          gr::io_signature::make2(
              2, M + 1, num_sc * M * sizeof(gr_complex), num_sc * sizeof(gr_complex)),
...
void apply_weights_impl::forecast(int noutput_items, gr_vector_int& ninput_items_required)
{
        ...
        for (int a = 0; a < d_num_avg; a++) {
            for (int sc = 0; sc < d_num_sc; sc++) {
                ...
                volk_32fc_x2_conjugate_dot_prod_32fc(
                    &out[s * d_num_avg * d_num_sc + d_num_sc * a + sc],
                    &d_sample[0],
                    &weights[s * d_num_sc * d_M + d_M * sc],
                    d_M);
            }
        }
    }
